...
- round 383 (XYZE photometric semantics, cont. — on-disk fixture): add
  ...
  round-192/196 fixtures all carry the default RGBE format, leaving the
  XYZE wire path (FORMAT record parse + emit, XYZE pixel round-trip)
  unpinned by committed bytes. Deterministic construction (Y ramps
  ~4 decades along the diagonal, X/Z track at fixed 0.9/1.2 ratios so
  chromaticity is constant) with `EXPOSURE=2` and the reference-manual
  `PRIMARIES=`; generated by the extended `examples/gen_fixtures.rs`
  (the four existing fixtures regenerate byte-identical). The matching
  `tests/fixture_decode.rs` case additionally anchors the round-383
  luminance fix at the file level: `luminance_buffer` equals the stored
  Y channel bit-for-bit (no 179×), `scene_referred_luminance_buffer`
  equals `Y / 2` bit-for-bit (the EXPOSURE=2 recovery is an exact
  power-of-two scale), the constant X/Y and Z/Y chromaticity ratios and
  ramp extremes survive the shared-exponent quantisation, and the
  decoded picture re-encodes byte-identical to the committed file
...
- round 378 (depth — old-RLE decode robustness): reject a leading
  ...
  unaffected. Three further `tests/roundtrip.rs` integration tests drive
  the same guard end-to-end through the shipped `parse_hdr` (default
  `FallbackMode::OldRle`): a first-scanline leading sentinel is rejected,
  a literal-then-sentinel first scanline decodes, and a multi-scanline run
  whose *second* row opens with a sentinel (repeating the carried-in
  previous pixel — the interop case our own encoder never emits) decodes
  to a uniform picture
...
- round 366 (depth — scene-referred radiance recovery subsystem): add `HdrImage::scene_referred_radiance_buffer` — a non-mutating `width*height*3` float buffer of the picture's scene-referred radiance, i.e. the stored channels with the cumulative `EXPOSURE=` multiplier and `COLORCORR=` triple the writer baked in divided back out, per the staged spec (`docs/image/hdr/radiance-hdr-rgbe-format.md` §1: "to recover original radiances (watts/sr/m²) divide file values by the product of all EXPOSURE settings", with the complementary per-primary `COLORCORR` division). This is the RGB-buffer counterpart to the round-323 `scene_referred_luminance_buffer` (which only returns the scalar 179·luminance projection): consumers that need scene-referred RGB for downstream radiometric work (custom luminance weighting, physically-based relight, a different colour-space projection) no longer have to mutate the image and clobber its typed `EXPOSURE=` / `COLORCORR=` slots via two separate `recover_original_radiance` / `recover_original_colorcorr` calls. The recovery factors are factored into a shared private `scene_referred_recovery_factors` helper so the radiance buffer and the luminance buffer are numerically identical by construction (a new test confirms the luminance of the recovered RGB equals `scene_referred_luminance_buffer` per pixel). Non-mutating — the buffer can be taken repeatedly and the slots survive for a re-encode; when neither record is present (or both are the identity) the returned buffer equals `pixels` exactly. A degenerate cumulative factor (`EXPOSURE` `0.0`/non-finite, any `COLORCORR` component `0.0`/non-finite) maps to the `1.0` identity for that factor — the same permissive handling the `recover_*` helpers use — so a malformed header can never inject NaN / ∞. Six new image-module tests pin the identity-without-records case, exposure-only and colorcorr-only recovery, the combined composition, the agreement-with-luminance-buffer invariant, and the degenerate-factor identity fallback
- round 354 (depth — geometric reorientation subsystem): turn the §2 resolution-string orientation table (`docs/image/hdr/radiance-hdr-rgbe-format.md`) into an actual in-memory picture-transform API on the decoded float buffer. New `GeometricOp` enum names the eight rigid symmetries of a rectangle (the dihedral group `D₄`): `Identity`, `FlipHorizontal`, `FlipVertical`, `Rotate180`, `Rotate90Cw`, `Rotate90Ccw`, `Transpose`, `AntiTranspose`. It exposes `ALL`, `swaps_dimensions` (the four 90°-class ops swap width/height), `inverse` (every op has one; the two 90° rotations invert to each other, the other six are involutions), and `then` (group composition via a rotation+mirror normal form, proven closed + associative). `Orientation::display_transform` / `from_display_transform` map each of the eight resolution-string forms to the `GeometricOp` that carries the **standard** displayed picture onto it (and back) — a bijection, with the four X-first orientations falling exactly on the four dimension-swapping ops. On `HdrImage` (whose `pixels` buffer is always canonical standard top-down/left-to-right display order): `apply_geometric(op)` rewrites the picture content in place (lossless pure pixel permutation — no radiance value altered — swapping `width`/`height` for the 90°-class ops, header/`pixel_format` untouched, degenerate zero-dimension pictures swap extents only); `to_orientation(o)` renders the buffer as orientation `o` would display it; `normalize_from(o)` undoes that; and `reorient(from, to)` moves content across the full 8×8 orientation matrix in a single permutation by collapsing `from⁻¹ ∘ to` into one `GeometricOp`. The two byte-level primitives plus methods are pinned by an independent coordinate-model ground truth: 8 header-module tests cover the group axioms (closure, associativity, inverse, the display-transform bijection) and 8 image-module tests cover the buffer permutation per op against the model, op-then-inverse bit-exact restoration, the buffer-level group law, the `to_orientation`/`normalize_from` round-trip, `reorient == normalize+to_orientation` across all 64 ordered pairs, `reorient(o,o)` identity, the zero-dimension guard, and a concrete quarter-turn sanity check
